...
int main()
{
	...
	Shader ourShader("shaders/vs.vs", "shaders/fs.fs");
	Shader lineShader("shaders/vs.vs", "shaders/line.fs");


	Scene scene;
	Model model = Model("./objects/cup.obj");
	std::unique_ptr<Entity> ourEntity = make_unique<Entity>(model, false);
...

	{
		...
		for (unsigned int y = 0; y < NUM; y++) {
			for (unsigned int x = 0; x < NUM; ++x)
			{
				...
				{
					ourEntity->addChild(model, false);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		scene.draw_non_instanced(ourShader, camFrustum);
		...
		//std::cout << "dir: " << camera.Front.x << " " << camera.Front.y << " " << camera.Front.z << std::endl;
