...
struct Plane
{
	...
	float     distance = 0.f;        // signed distance with origin: the distance of the origin to the plane along plane normal dir
...
public:
	...
	bool isOnOrAbovePlane(const Plane& plane) const final
	{
		...
		// extents is always non negative,for plane normal, and we only care about the magnitude
		// in normal dir.
